...
void ag71xx_add_ar7240_header(struct sk_buff *skb, u8 ath_hdr_port_byte){
  ...
  if(unlikely(skb_headroom(skb) < ATHEROS_HEADER_LEN)){
		if(printk_ratelimit())
			printk(KERN_ERR "%s: critical error! no free space in skb headroom!\n",
						 __func__);
		return;
	}
	__skb_push(skb, ATHEROS_HEADER_LEN);
  //записываем Atheros header
	skb->data[0] = ath_hdr_port_byte;
	skb->data[1] = 0x40;
	//skb_16b_ddd(skb);
}
...
int ag71xx_remove_ar7240_header(struct sk_buff *skb, int pktlen, int *port_num){
	...
	*port_num = skb->data[0];
	//skb_16b_ddd(skb);
	skb_pull(skb, ATHEROS_HEADER_LEN);
	return 0;
}
...
void ag71xx_add_ar934x_header(struct sk_buff *skb, u8 ath_hdr_port_byte){
  ...
  if(unlikely((void*)skb_tail_pointer(skb) < end + ATHEROS_HEADER_LEN)){
		if(printk_ratelimit())
